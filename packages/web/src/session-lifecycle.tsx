  ...
  return <div className="control-scrim" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="session-lifecycle" ref={panel} role="dialog" aria-modal="true" aria-labelledby="session-lifecycle-title" onKeyDown={trapFocus}>
      ...
      <div className="lifecycle-actions">
        <button title={capabilities.has("session.clone") ? undefined : "Unavailable because session clone was not granted"} disabled={busy || operationActive || !capabilities.has("session.clone")} onClick={onClone}><span><strong>Clone session</strong><small>Create a complete independent copy.</small></span><b aria-hidden="true">›</b></button>
        <button title={capabilities.has("session.export") ? undefined : "Unavailable because session export was not granted"} disabled={busy || operationActive || !capabilities.has("session.export")} onClick={onExport}><span><strong>Export artifact</strong><small>Download durable history and attachments.</small></span><b aria-hidden="true">↓</b></button>
        <button title={capabilities.has("session.dispose") ? undefined : "Unavailable because session disposal was not granted"} disabled={busy || ["inactive", "disposing"].includes(session.runtime.state) || !capabilities.has("session.dispose")} onClick={onDispose}><span><strong>End runtime</strong><small>Stop execution while preserving durable history.</small></span><b aria-hidden="true">■</b></button>
