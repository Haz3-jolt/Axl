...
Configure global servers in `~/.axl/mcp.json` or project servers in `<workspace>/.axl/mcp.json`. Project entries replace global entries with the same name. Setting a project entry to `"enabled": false` disables the matching global server completely.
...
Local stdio servers run through Axl's required OS sandbox with a read-only host, workspace-scoped writes, protected Axl state, a cleared environment, and no network. `roots` are opt-in and must resolve inside the active workspace.
...
Binary content is stored by SHA-256 digest under `~/.axl/blobs`, and the event log contains only the reference. Axl reports MCP errors directly. If the active model cannot accept a requested media type, the sampling request fails instead of converting or dropping the input.
